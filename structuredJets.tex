...
\begin{equation}
	\dot{\theta_0} = \frac{c_s}{\gamma R}\ , \qquad \text{if } u < 3/\theta_0 \text{ and } \theta_0 > \pi/2\ . \label{eq:thetadot}
\end{equation}
The criterion $u < 3/\theta_0$ is satisfied when enough time has elapsed that a sound wave can propagate from $\theta = \theta_0$ to $\theta=0$ in a blast wave propagating in an ISM (CITE HENDRIK).  It is similar to the condition given in \cite{Duffell:2018aa}, which found a numerical factor of 2.5 (instead of 3) after calibrating to numerical simulations.  Our criteria differs from that used in (CITE Troja+) which used an overly simplistic $u < 1$ criterion to begin jet spreading.
...
Bayesian parameter estimation utilizing \afterglowpy{} and a Markov chain Monte Carlo sampler was performed in \cite{van-eerten:2018aa}.  The conclusions were very similar to the above analytic estimates, inferring for a Gaussian jet model $\thobs = 0.52\pm0.16$ and $\thC=0.08\pm0.02$.  Incorporating the LIGO posterior for $\thobs$, assuming the Planck value for $H_0$, tightens the constraints to $\thobs = 0.38\pm0.11$ and $\thC=0.06\pm0.02$.  Utilizing the full numerical \afterglowpy{} model allows $E_0$, $n_0$, $p$, $\epse$, and $\epsB$ to also be inferred, although given the lack of observed spectral features in the GW170817 afterglow the resulting constraints are quite broad.
...
Many groups have used structured jet models to analyze the afterglow of GW170817A \citep{Mooley:2018ab, Ghirlanda:2019aa, Lamb:2019aa, Wu:2018aa}.
