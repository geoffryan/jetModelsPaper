...
Each fiducial model is parameterized by a normalization $E_0$, a width $\thC$, and a truncation angle $\thW$.  The power-law model also retains a power-law index $b$.  Outside $\thW$ the energy is initially zero, this acts as an outer truncation angle for the jet.   Figure \ref{fig:jetStruct} shows an example of each model fit to recent simulation results.  The Gaussian model emulates jets with a flat core and sharp edges (e.g. CITE Margutti), while the power-law emulates jets with narrow cores and large wings (AWK. jargon? CITE Lazzati, Mizuta).  
...
\begin{equation}
	\Delta \Omega = \gamma^{-s_\Omega} \ .
\end{equation}
The parameter $s_\Omega$ controls the growth rate of the visible patch, and is typically between 0 and 2.
...
It is important to note that when $\thobs \neq 0$ the transition period is extended, and the most significant transition in the light curve occurs at $\tbp$, the \emph{jet break}.  The jet break occurs, in all jets, when the effective angular size of the visible patch of the jet ceases to increase, that is when $s_\Omega = 0$.  
...
\begin{deluxetable*}{CCCCCCC}
	...
	\startdata
	\nu<\nu_m<\nu_c     & D & -7 			& -\frac{5/2 + 3g}{8+g} 		& -3		& 1/4			& -1/3 \\
	\nu_m<\nu<\nu_c     & G & -15/2 + 3p/2 	& \frac{-9/2 + 6p - 3g}{8+g} 	& -3		& 3p/4		& (p-1)/2 \\
	\nu < \nu_c < \nu_m & E & -17/3 		& \frac{1/6 - 11g/3}{8+g} 		& -11/3	& 7/12		& -1/3 \\
	\nu_c < \nu < \nu_m & F & -13/2 		& \frac{7/2 - 2g}{8+g} 		& -2		& 1			& 1/2 \\
	\nu_c < \nu_m < \nu & H & -8 + 3p/2 		& \frac{-5/2 +6p  -2g}{8+g} 	& -2		& (3p+1)/4	& p/2 \\ 
	\enddata
\end{deluxetable*}
...
A blast wave will have an initial coasting period before sweeping up enough mass in the ambient medium to begin deceleration and be subject to refreshed shocks.  The shock refreshment ends when the blast wave decelerates past $\umin$.  In the single shell model with trans-relativistic equation of state used here, these times are:
\begin{align}
...
The light curves of both the angularly structured jet and refreshed shock models can be understood in terms of general energy injection mechanisms, which are typically parameterized as an additional power $L$ delivered to the blast wave.  This power is typically taken to decay in time as a power law, $L \propto t^{-q}$, with $q \in [0,1]$.

The mapping between energy injection and refreshed shocks was done in (CITE Zhang06).  That work specifies the index $s$ of the mass distribution $M_{>u} \propto u^{-s}$, which is related simply to the index $k$ by $k = s-1$. Under this mapping the closure relations in Table \ref{tab:slopes} are equivalent to those in (CITE Zhang06) and the model can be seen as equivalent to an energy injection model with $q = (8-2k) / (8+k)$.

There is no single mapping between the structured jet and energy injection or refreshed shock models.  However, a visual comparison between $\alpha_{\text{struct}}$ in Table \ref{tab:slopes} and $\alpha_{\text{refresh}}$ in Table \ref{tab:refreshSlopes} indicate the identical roles played by $g$ and $k$.  Indeed, a simple swap $g \leftrightarrow k$ produces almost identical closure relations.  However, as seen in Tables \ref{tab:slopes} and \ref{tab:refreshSlopes} or 

The mapping between structured jet and refreshed shock (and hence energy injection) models is a two step process.   Indeed, a stru
...
The numerical model described in Section \ref{sec:numerical} improves on these analytic approximations by ...
...
\begin{deluxetable*}{CCCCCC}
	\tablecaption{Generic structured phase temporal and spectral slopes: $F_\nu \propto \tobs^{-\alpha} \nu^{-\beta}$.  The visible patch of the jet has a solid angle which scales as $\gamma^{-s_3}$. See Equations \eqref{eq:gdef} and \eqref{eq:geff} for the definition and effective values of $g$.  A uniform relativistic blast wave pre-jet break has $s_3=2$, a relativistic blast wave with angular structure pre-jet break has $s_3 \approx 1.5$, and a relativistic blast wave post jet-break has $s_3 = 0$. For a refreshed-shock model replace $g$ with $k$, defined in Equation \eqref{eq:Eu}. \label{tab:generalSlopes}}
	\tablehead{\colhead{Regime}& \colhead{Label} & \colhead{$\alpha_{\mathrm{struct}}$} & \colhead{$\beta$}}
	\startdata
	\nu<\nu_m<\nu_c     & D & \frac{2-3s_3 - 3g}{8+g} 		& -1/3 \\
	\nu_m<\nu<\nu_c     & G & \frac{6p - 3s_3 - 3g}{8+g} 	& (p-1)/2 \\
	\nu < \nu_c < \nu_m & E & \frac{14/3 - 3s_3 -11g/3}{8+g} 	& -1/3 \\
	\nu_c < \nu < \nu_m & F & \frac{8-3s_3 -2g}{8+g} 		& 1/2 \\
	\nu_c < \nu_m < \nu & H & \frac{6p - 2 - 3s_3 - 2g}{8+g} 	& p/2 \\ 
	\enddata
\end{deluxetable*}
...
Figure \ref{fig:170817Alc} shows the observations in radio, optical, and x-ray bands of the GW170817A afterglow.  No afterglow emission was detected until an x-ray detection by the Chandra observatory nine days after the GW trigger (CITE TrojaNature).  Further radio, optical, and x-ray observations showed a steady rising light curve with constant power-law slope until a peak near day 160.  At this point the light curve turned over and began a steep decay that continues to the present (CITE YearInTheLife).
