...
% TODO FIX THE HELL OUT OF THESE FIGURES
% MAKE MODEL CURVES CONSISTENT WITH MULTI-PANEL
% DROP OR DEFINE t_N
%\begin{figure}
%	\includegraphics[width=\columnwidth]{figs/jetbreak_OnAxis.pdf}
%	\caption{Jet break time for on-axis ($\thobs=0$) observers for Gaussian (green solid line),  $b=2$ power law (orange dash-dotted line), and $b=6$ power law (red dotted line) jets as a function of $\thC$. The dashed grey line is the analytic approximation $\tb = 1.55 t_N (\thC/1 rad)^{8/3}$.  \label{fig:tbOA}}
%\end{figure}
...
\begin{enumerate}
	...
	\item In the context of a Gaussian jet, the measurement of $\geff$ leads immediately to $\thobs / \thC = 5.7 \pm 0.2$ using Equation \eqref{eq:geff}.  A $b=2$ power law jet cannot achieve this $\geff$ for any reasonable value of $\thobs / \thC$.  A $b=6$ power law jet requires $\thobs/\thC = 14 \pm 1$.
\end{enumerate}
...
%TODO FINISH THIS PARAGRAPH
Analysis of the \gwbns{} gravitational wave signal produced estimates of the inclination angle $\iota$ between the binary neutron stars' orbital plane and our line of sight.


This is in agreement with the LIGO measurement $\thobs \approx 28^\circ \pm 15^\circ$ (assuming the SHoES $H_0$).
