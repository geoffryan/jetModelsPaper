...
def t_on(thV, thC, thW, E0, n0, p, jetType=0):
    ...
    return t0 * math.pow(2,-1.6) * math.pow(2*math.sin(0.5*(thV-thW)), 8.0/3.0)
...
def plot_lc_cocoon(t, umax, umin, Er, k, Mej, n0, p, jetType=3, seg='G'):
    ...
    nu = np.empty(t.shape)
    nu[:] = 1.0e14
    Y = np.array([umax, umin, Er, k, Mej, 0, 0, 0, n0, p, 0.1, 1.0e-4, 1.0, 10])
    FnuSA = grb.fluxDensity(t, nu, jetType, 0, *Y)
...
def prettyLCAxis(ax, tmin, tmax, Fmin, Fmax):
    ...
    #ax.set_title('$E_0=10^{52}\mathrm{erg}\ n_0 = 10^{-3}\mathrm{cm}^{-3}\ \\theta_0=0.1\ \mathrm{rad} \ \\theta_{obs} = 0.5\ \mathrm{rad}$', fontsize=16)
...
def makeNicePlot(t, thV, thC, thW, E0, n0, p, jetType, seg):
    ...
    ax.plot(t[phase2]/day, FA[phase2], lw=4, color='r', alpha=1.0, label=labelSJ)
    ...
    ax.plot(t/day, FN, lw=2, color='k', ls='--', label=labelN)
    ax.plot(t/day, FA, lw=4, color='k', alpha=0.5, label=labelPL)
    ax.plot(t[phase2]/day, FA[phase2], lw=4, color='r', alpha=0.5, label=labelSJ)
