...
def plotPanel(fig, gs0, t, nu, thC, Y, Z, modelLegend=False):
    ...
    axF.plot(t, FnuPL1, label='Power Law Jet $b=3$', color=c[2], ls=ls[2])
    ...
    axF.text(0.95, 0.95,
             ...
             + '\n' + r'$\theta_C = $ {0:.2f} rad'.format(thC)
             + '\n' + r'$\theta_W = $ {0:.2f} rad'.format(thW),
...
plotPanel(fig, gs[0, 0], t, nu, 0.04, Y0, Z)
plotPanel(fig, gs[0, 1], t, nu, 0.08, Y0, Z)
plotPanel(fig, gs[1, 0], t, nu, 0.12, Y0, Z, modelLegend=True)
plotPanel(fig, gs[1, 1], t, nu, 0.16, Y0, Z)

fig.tight_layout()

figname = "lc_thC_model_multi.pdf"
